...
void setup()
{
  ...
  Firebase.setString("Test", "Coriander");
  initTemp();
  WiFi.mode(WIFI_STA);
...
void smokesensor()
{
  ...
  Firebase.setFloat("Smoke_C",gasvalue);
  if(gasvalue>1200)
  {
    Serial.print("Smoke Detected");
    Firebase.setString("Smoke_C_Alert", "Smoke_Detected");
  }
  else
  {
    Serial.print("Smoke Not Detected");
    Firebase.setString("Smoke_C_Alert", "Smoke_Not_Detected");
  }
  Serial.println();
}
...
void pirsensor()
{
  ...
  Firebase.setFloat("pir_C",pirvalue);
  if(pirvalue==1)
  {
    Serial.print("Motion Detected");
    SendMessage1();
    SendMessage4();
    Firebase.setString("Pir_Alert_C", "Motion_Detected");
  }
  else
  {
    Serial.print("Motion Not Detected");
    Firebase.setString("Pir_Alert_C", "Motion_Not_Detected");
  }
  Serial.println();
}
...
void soilsensor()
{
  ...
  Firebase.setFloat("Soil_C",soilvalue);
  if(soilvalue>3000)
  {
    Serial.print("Normal Detected");
    SendMessage2();
    SendMessage5();
    Firebase.setString("Moisture_Alert_C", "Normal_Water_Detected");
  ...
  else
  {
    ...
    Firebase.setString("Moisture_Alert_C", "High_Water_Detected");
    digitalWrite(relay, LOW);
  }
  Serial.println();
}
...
void rainsensor()
{
  ...
  Firebase.setFloat("Rain_C",rainvalue);
  if(rainvalue<1700)
  {
    Serial.print("Rain Detected");
    Firebase.setString("Rain_Alert_C", "Rain_Detected");
  }
  else
  {
    Serial.print("Rain Not Detected");
    Firebase.setString("Rain_Alert_C", "Rain_Not_Detected");
  }
  Serial.println();
}
...
void irsensor()
{
  ...
  Firebase.setFloat("Ir_C",irvalue);
  if(irvalue==0)
  {
    Serial.print("Object Detected");
    Firebase.setString("Ir_Alert_C", "Object_Detected");
  }
  else
  {
    Serial.print("Object Not Detected");
    Firebase.setString("Ir_Alert_C", "Object_Not_Detected");
  }
  Serial.println();  
}
...
void ldrsensor()
{
  ...
  Firebase.setFloat("LDR_C",ldrvalue);
  if(ldrvalue==1)
  {
    Serial.print("Dark Detected");
    Firebase.setString("LDR_Alert_C", "Dark_Detected");
  }
  else
  {
    Serial.print("Light Detected");
    Firebase.setString("LDR_Alert_C", "Light_Detected");
  }
  Serial.println();  
}
    ...
    if (status != 0) {
      ...
      if (status != 0) {
        ...
        if (status != 0) {
          ...
          Serial.print(P);
          Serial.println(" hPa");
          Firebase.setFloat("Pressure_C",P);
        }
      }
    }
}
...
delay(5000);
  if(Serial.available()>0)
   {
    count=0;
    while(Serial.available() && count < 26)
    {
   ...
   Firebase.setString("leaf disesse alert_c",input); 
  if (Firebase.failed()) {
  ...
  temperature();
  Firebase.setFloat("Temperature_C",Tempi);
  Firebase.setFloat("Humidity_C",Humii);
